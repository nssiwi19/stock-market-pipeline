...
def main() -> None:
    ...
    heartbeat = out_dir / f"batch150_rebalance_{run_id}.heartbeat.txt"
    result_file = out_dir / f"batch150_rebalance_{run_id}.result.json"
    error_file = out_dir / f"batch150_rebalance_{run_id}.error.txt"
    done_file = out_dir / f"batch150_rebalance_{run_id}.done.txt"
    ...
    try:
        result = run_pilot(
            ...
            doc_parse_per_ticker=1,
            doc_top_terms=6,
            doc_page_size=12,
            allow_new_key_insert=True,
            min_new_key_fields=4,
            min_new_key_confidence=0.85,
            new_key_allowed_sources="vietstock_financeinfo,vci_financial_api",
            allow_ocr_source=True,
            ocr_debug=False,
            ocr_debug_tickers="",
            ocr_debug_max_lines=120,
            ocr_debug_max_files_per_ticker=2,
            ocr_source_tickers="",
            ocr_source_top_missing=30,
        )
        payload = {
            "run_id": run_id,
