...
def main() -> None:
    ...
    heartbeat = out_dir / f"pilot20_pdf_{run_id}.heartbeat.txt"
    heartbeat.write_text(f"started_at={started.isoformat()}\n", encoding="utf-8")

    result_file = out_dir / f"pilot20_pdf_{run_id}.result.json"
    error_file = out_dir / f"pilot20_pdf_{run_id}.error.txt"
    done_file = out_dir / f"pilot20_pdf_{run_id}.done.txt"
    try:
        result = run_pilot(
            limit=20,
            sleep_ms=0,
